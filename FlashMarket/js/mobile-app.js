...
(() => {
  ...
  function removeMobileDrawer() {
    document.querySelectorAll('.mobile-menu-drawer, .mobile-menu-overlay').forEach(element => element.remove());
    document.body.classList.remove('mobile-menu-open');
  }

  function disableOldMenuButton() {
  ...
  function injectSmallPolish() {
    ...
    style.textContent = `@media(max-width:700px){.mobile-menu-drawer,.mobile-menu-overlay{display:none!important}.mobile-toggle{display:none!important}.mobile-app-nav{touch-action:manipulation}}`;
    document.head.appendChild(style);
  }

  function init() {
    removeMobileDrawer();
    disableOldMenuButton();
    createNav();
