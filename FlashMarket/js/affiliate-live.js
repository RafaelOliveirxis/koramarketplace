...
(() => {
  ...
  function setMessage(text, ok = false) {
    ...
    if (el) { el.textContent = text; el.style.color = ok ? '#98f5c2' : '#ffc21c'; }
  }
  ...
  function setScreen(logged) {
    ...
    const benefits = document.querySelectorAll('.auth-benefits');
    benefits.forEach(el => { el.hidden = true; el.style.display = 'none'; });
    document.body.classList.toggle('affiliate-live-page', true);
    document.body.classList.toggle('logged-screen', logged);
  ...
  function updateLink(code) {
    ...
    if ($('affiliateLink')) $('affiliateLink').textContent = `${base}index.html?af=${encodeURIComponent(code)}`;
  }
  ...
  function init() {
    ...
    const hasAuth = !!token() && localStorage.getItem('flashmarket_affiliate_session') === 'true';
    if (hasAuth) setScreen(true); else setScreen(false);
    addRefreshControl();
